...
int main( int const argc, char* argv[] )
{
    ...
    for( auto const& item : run_config.items )
    {
        ...
        cl::Buffer fourier_coeff( ctx, CL_MEM_READ_WRITE,
            sizeof( real_type ) * CERRF_N_FOURIER, nullptr );

        cl::Buffer taylor_coeff( ctx, CL_MEM_READ_WRITE,
            sizeof( real_type ) * CERRF_NUM_TAYLOR_COEFF, nullptr );
        ...
        ret = queue.enqueueWriteBuffer( fourier_coeff, CL_TRUE,
            std::size_t{ 0 }, sizeof( real_type ) * CERRF_N_FOURIER,
                &CERRF_FOURIER_COEFF[ 0 ] );

        assert( ret == CL_SUCCESS );

        ret = queue.enqueueWriteBuffer( taylor_coeff, CL_TRUE,
            std::size_t{ 0 }, sizeof( real_type ) * CERRF_NUM_TAYLOR_COEFF,
        ...
        kernel.setArg( 4, nn );
        kernel.setArg( 5, fourier_coeff );
        kernel.setArg( 6, taylor_coeff );
